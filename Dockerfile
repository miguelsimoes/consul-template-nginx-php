FROM smsimoes/consul-template-php:7.1
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -qq libssl1.0.0 php7.1-fpm php7.1-apcu php7.1-apcu-bc -y
...
RUN sed -i -e "s/;realpath_cache_size\*=\*4096k/realpath_cache_size=4096K/g"                 /etc/php/7.1/fpm/php.ini
RUN sed -i -e "s/;realpath_cache_ttl\*=\*120/realpath_cache_ttl=600/g"                       /etc/php/7.1/fpm/php.ini
...
RUN sed -i -e "s/;opcache.revalidate_freq=2/opcache.revalidate_freq=60/g"                    /etc/php/7.1/fpm/php.ini
RUN sed -i -e "s/;opcache.fast_shutdown=0/opcache.fast_shutdown=1/g"                         /etc/php/7.1/fpm/php.ini
RUN sed -i -e "s/;daemonize\s*=\s*yes/daemonize=no/g"                                        /etc/php/7.1/fpm/php-fpm.conf
RUN sed -i -e "s/pm.max_children\s*=\s*5/pm.max_children=9/g"                                /etc/php/7.1/fpm/pool.d/www.conf
RUN sed -i -e "s/;opcache.max_accelerated_files=4000/opcache.max_accelerated_files=20000/g"  /etc/php/7.1/fpm/php.ini
